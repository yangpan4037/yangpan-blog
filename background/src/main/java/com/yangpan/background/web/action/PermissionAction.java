...
import com.yangpan.background.query.PermissionQuery;
...
public class PermissionAction extends CRUDAction<Permission> {
	private static final long serialVersionUID = 2928067470642145934L;
	...
	public void prepareInput() throws Exception {
		if (id != null) {
			permission = permissionService.get(id);//回显
		}
	}
	...
	public void prepareSave() throws Exception {
		if (id != null) {
			permission = permissionService.get(id);//保证属性不丢失关键代码
		} else {
	...
	public String changeName() throws Exception {
		...
		if (pk != null && value != null) {
			Permission tempPermission = permissionService.get(pk);
			String tempName = tempPermission.getName();
			if (tempName != null && tempName.equals(value)) {
				out.print("{\"msg\":true}");
			} else {
				tempPermission.setName(value);
				permissionService.saveOrUpdate(tempPermission);
			}
		}
		return NONE;
	}
	...
	public String getDownloadFileName() throws UnsupportedEncodingException {
		return new String("permission.xlsx".getBytes("GBK"), "ISO8859-1");
	}
	...
	public String download() throws Exception {
		...
		//得到查询的数据
		List<Permission> data = pageList.getData();
		//遍历并把数据封装
		for (Permission permission : data) {
			String[] strings = new String[heads.length];
			strings[0] = permission.getName();
			list.add(strings);
		}
		//使用因为struts2做下载，必须提供一个InputStream的对象
		this.inputStream = permissionService.download(list, heads);
		return "download";
	}
	 ...
	 * 所以需要子类需要覆写这个方法，他的好处是，把permission进行了压栈，放到值栈的最顶部，然后jsp可以直接使用permission的属性，不必像以前那样permission.id
	 */
	@Override
